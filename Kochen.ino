...
State stateReachKochTemp = State(prepareReachKochTemp, reachKochTemp, prepareKochDauer);
...
void defineHopfengaben() {
  ...
  if (encoderValue != 0 || first) {
    ...
    Serial.print(hopfengaben[aktuelleHopfengabe]);
...
void waitKochDauer() {
  ...
  if (anzahlHopfengaben - 1 > alertedHopfengaben) {
    long hg = hopfengaben[alertedHopfengaben] * 60 * 1000L;
...
void alertHopfengabe() {
  if (first) {
    ...
    snprintf(lcdBuf, sizeof(lcdBuf), "%i. Hopfeng.\0", (alertedHopfengaben));
    lcd.print(lcdBuf, CENTER, 16);

    snprintf(lcdBuf, sizeof(lcdBuf), "%i min. Kochz.\0", hopfengaben[alertedHopfengaben-1]);
    lcd.print(lcdBuf, CENTER, 24);
  }
